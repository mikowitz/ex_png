defmodule ExPng.Image.LineTest do
  ...
  describe "to_pixels" do
    test "it returns the correct pixels for a 1bit grayscale image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn0g01.png") do
        check_pixel_dimensions(image)

        image = ExPng.Image.to_lines(image)
        line = Enum.at(image.lines, 0)
        pixels = Line.to_pixels(line, image.header.bit_depth, image.header.color_type)
    ...
    test "it returns the correct pixels for a 2bit grayscale image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn0g02.png") do
        check_pixel_dimensions(image)
    ...
    test "it returns the correct pixels for a 4bit grayscale image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn0g04.png") do
        check_pixel_dimensions(image)
    ...
    test "it returns the correct pixels for a 8bit grayscale image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn0g08.png") do
        check_pixel_dimensions(image)
      end
    end

    test "it returns the correct pixels for a 16bt grayscale image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn0g16.png") do
        check_pixel_dimensions(image)
      end
    end

    test "it returns the correct pixels for a 8bit truecolor image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn2c08.png") do
        check_pixel_dimensions(image)
      end
    end

    test "it returns the correct pixels for a 16bit truecolor image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn2c16.png") do
        check_pixel_dimensions(image)
      end
    end

    test "it returns the correct pixels for a 1bit indexed image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn3p01.png") do
        check_pixel_dimensions(image)
      end
    end

    test "it returns the correct pixels for a 2bit indexed image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn3p02.png") do
        check_pixel_dimensions(image)
      end
    end

    test "it returns the correct pixels for a 4bit indexed image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn3p04.png") do
        check_pixel_dimensions(image)
      end
    end

    test "it returns the correct pixels for a 8bit indexed image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn3p08.png") do
        check_pixel_dimensions(image)
      end
    end

    test "it returns the correct pixels for a 8bit grayscale alpha image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn4a08.png") do
        check_pixel_dimensions(image)
      end
    end

    test "it returns the correct pixels for a 16bit grayscale alpha image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn4a16.png") do
        check_pixel_dimensions(image)
      end
    end

    test "it returns the correct pixels for a 8bit truecolor alpha image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn6a08.png") do
        check_pixel_dimensions(image)
      end
    end

    test "it returns the correct pixels for a 16bit truecolor alpha image" do
      with {:ok, image} <- ExPng.from_file("test/png_suite/basic/basn6a16.png") do
        check_pixel_dimensions(image)
      end
    end
  end

  defp check_pixel_dimensions(image) do
    image = ExPng.Image.to_canvas(image)
    assert length(image.lines) == image.header.height
    for pixels <- image.pixels do
      assert length(pixels) == image.header.width
    end
  end
end
