defmodule ExPng.Image.Adam7 do
      ...
      Enum.reduce(passes, {0, [], data.data}, fn [w, h], {pos, lines, data} ->
        ...
        if w * h > 0 do
          ...
          pixels =
            ...
            |> Enum.map(
              &(Pixelation.to_pixels(&1, bit_depth, color_mode, palette, transparency)
                |> Enum.take(w))
            )
  ...
  def decompose_into_sub_images(image) do
    for pass <- 0..6 do
      ...
      ys =
        Stream.iterate(y_offset, &(&1 + (1 <<< y_shift)))
        |> Enum.take_while(fn y -> y < image.height end)

      xs =
        Stream.iterate(x_offset, &(&1 + (1 <<< x_shift)))
        |> Enum.take_while(fn x -> x < image.height end)
