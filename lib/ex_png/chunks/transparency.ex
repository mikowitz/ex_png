defmodule ExPng.Chunks.Transparency do
  ...
  @spec parse_data(ExPng.maybe(__MODULE__.t), Header.t) :: {:ok, ExPng.maybe(__MODULE__.t)} | {:error, binary()}
  ...
  def parse_data(%__MODULE__{data: data} = transparency, %Header{color_mode: 0, bit_depth: bit_depth}) do
    gray =
      data
  ...
  def parse_data(%__MODULE__{data: data} = transparency, %Header{color_mode: 2, bit_depth: bit_depth}) do
    <<r::bytes-size(2), g::bytes-size(2), b::bytes-size(2)>> = data

    [r, g, b] = [r, g, b] |> Enum.map(&:binary.decode_unsigned/1)

    transparent_pixel = case bit_depth do
      1 ->
